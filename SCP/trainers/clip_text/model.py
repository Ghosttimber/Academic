...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        clone_feature = x[:, 0, :].detach().clone()
        x = self.ln_post(x[:, 0, :])
...
def save_tensor_with_incremental_filename(tensor, base_filename):
    # 获取文件名和扩展名
...
class CLIP(nn.Module):
    def __init__(
        ...
        self.transformer = TextTransformer(width=transformer_width,
    ...
    def encode_text(self, text):
        ...
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        clone_feature = x[torch.arange(x.shape[0]), text.argmax(dim=-1)].detach().clone()
        #save_tensor_with_incremental_filename(x, 'full_textFeature.pt')
