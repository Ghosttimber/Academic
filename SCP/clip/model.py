...
def matrixdistancebig(token,feature,lambda_factor=0.8):
    cos = torch.nn.CosineSimilarity(dim=2,eps=1e-07)
    
    token=token.float()
    feature=feature.float()

    token = token / token.norm(dim=2, keepdim=True)
    feature = feature / feature.norm(dim=2, keepdim=True)  
                            
    # distance = torch.norm(token - feature,p=2).pow(2)
    # score = expdistance(distance)

    score= cos(token,feature)
    score  = 1.0-torch.mean(score,dim=1)
    return score/2

def matrixdistance(token,feature,lambda_factor=0.8):
...
def expdistance(A,lambda_factor=0.1):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                ...
                x = torch.cat([prefix, visual_context.to(device=x.device)], dim=0)
            else:
                ...
                # layer learnable tokens
                x = torch.cat([prefix, textual_context.to(device=x.device), suffix], dim=0)
...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward(self, inputs):
        ...
        counter = inputs[2]
        embeddingx=inputs[3]
        ind_len=compound_prompts_deeper[0].shape[0]

        zero=embeddingx[0].shape[1]
        
        compound_prompt_nctx=ind_len+zero
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        embedding=embeddingx[0]
                        vision_fuxi=embeddingx[-1][counter]
                        embeddingLayers=adaptFeature(embedding.to(torch.float32),vision_fuxi)

                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :]
                        # Create/configure learnable tokens of this layer
                        visual_context = compound_prompts_deeper[counter]  # extract the correct index
                        visual_context = visual_context.expand(x.shape[1], -1, -1)
                        #visual_context=self.calentropY(embeddingx,visual_context.to(embeddingx.device)) #
                        
                        
                        ######################
                        # token= x[1+ ind_len:1+ compound_prompt_nctx,:,:]
                        # # token= x[1:1+ compound_prompt_nctx,:,:]  #x:torch.Size([199, 1, 768])  
                        
                        # token=token.permute(1, 0, 2).half()   # token output [1, 2, 768])
                        # score= matrixdistance_image(token,embeddingx)
                        
                        # if score <beta(0):
                        #         A_prime = token.clone()                                    
                        #         A_prime  = embeddingx.to(A_prime.dtype)
                        #         token = A_prime 
                                
                        new=torch.cat([visual_context ,embeddingLayers.to(visual_context.dtype)], dim=1)
                        new=new.permute(1, 0, 2).half()
                        
                        x = torch.cat([prefix,new], dim=0)
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        embedding=embeddingx[0]
                        text_fuxi=embeddingx[-1][counter]
                        embeddingLayers=adaptFeature(embedding.to(torch.float32),text_fuxi)
                        # Appending the learnable tokens in different way
                        # x -> [77, NCLS, DIM]
                        # First remove the learnable tokens from previous layer
                        prefix = x[:1, :, :]
                        suffix = x[1 + compound_prompt_nctx:, :, :]
                        # Create/configure learnable tokens of this layer
                        textual_context = compound_prompts_deeper[counter]
                        textual_context = textual_context.expand(x.shape[1], -1, -1)
                        ##textual_context=self.calentropY(embeddingx,textual_context)
                        # 替代
                        
                        # ######################
                        # token= x[1+ ind_len:1+ compound_prompt_nctx,:,:]
                        # # token= x[1:1+ compound_prompt_nctx,:,:] # X ([77, 50, 512])

                        # token=token.permute(1, 0, 2).half() # token result [50, 2, 512]

                        # score= matrixdistance(token,embeddingx)
                        
                        
                        # if score <alphaX(0):
                        #     # Index=[]
                        #     # for i in range(token.shape[0]):
                        #     #     if matrixdistance(token[i],embeddingx[i])  <alphaX(0):
                        #     #         A_prime = token[i].clone()
                        #     #         A_prime  = embeddingx[i].to(A_prime.dtype)
                        #     #         token[i] = A_prime 
                                    
                        #     A_prime = token.clone()
                        #     A_prime  = embeddingx.to(A_prime.dtype)
                        #     token = A_prime                                     
                        new=torch.cat([textual_context ,embeddingLayers.to(textual_context)], dim=1)
                        new=new.permute(1, 0, 2).half()
                        

                        
                        x = torch.cat([prefix, new, suffix], dim=0)
                        counter += 1
        ...
        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        return [x, compound_prompts_deeper, counter,embeddingx]  # return again as a list, so that nn.seq can work

# class TransformerO(nn.Module):
#     def __init__(self,
#                  width: int,
#                  layers: int,
#                  heads: int,
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, condition_feature_):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        ...
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        embeddingimage=x.clone().detach()
        # embeddingimage=self.calentrop(embeddingimage.to(x.device))
        x = x + self.positional_embedding.to(x.dtype)
        ...
        if self.VPT_shallow:
            ...
            #visual_ctx=self.calentropY(embeddingimage,visual_ctx).half()
            #visual_ctx=adaptFeature(condition_feature_,visual_ctx).half()
            x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        outputs = self.transformer([x, compound_deeper_prompts, 0,condition_feature_])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
        clone_feature = x[:, 0, :].clone()
        x = self.ln_post(x[:, 0, :])
...
class VisionTransformerO(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int,
        ...
        self.transformer = TransformerO(width, layers, heads)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        #save_tensor_with_incremental_filename(x, 'full_imageFeature.pt')
        x = self.ln_post(x[:, 0, :])
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        # self.visualO = VisionTransformerO(input_resolution=image_resolution,
        #                                     patch_size=vision_patch_size,
        #                                     width=vision_width,
        #                                     layers=vision_layers,
        #                                     heads=vision_heads,
    ...
    def initialize_parameters(self):
        ...
        # if isinstance(self.visualO, ModifiedResNet):
        #     if self.visualO.attnpool is not None:
        #         std = self.visualO.attnpool.c_proj.in_features ** -0.5
        #         nn.init.normal_(self.visualO.attnpool.q_proj.weight, std=std)
        #         nn.init.normal_(self.visualO.attnpool.k_proj.weight, std=std)
        #         nn.init.normal_(self.visualO.attnpool.v_proj.weight, std=std)
        #         nn.init.normal_(self.visualO.attnpool.c_proj.weight, std=std)

        #     for resnet_block in [self.visualO.layer1, self.visualO.layer2, self.visualO.layer3, self.visualO.layer4]:
        #         for name, param in resnet_block.named_parameters():
        #             if name.endswith("bn3.weight"):
        #                 nn.init.zeros_(param)
